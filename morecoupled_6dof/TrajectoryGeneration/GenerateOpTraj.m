...
function landervarsfun(sh, c)
    ...
    Fmax = 20;
    % Define Controls
    sh.addControl('u1', 'lb', -20, 'ub', 20);  % Force [N]
    sh.addControl('u2', 'lb', -20, 'ub', 20);  % Force [N]
    sh.addControl('u3', 'lb',   0, 'ub', 20);  % Force [N]
...
function landereqfun(sh,x,~,u,c) % https://charlestytler.com/quadcopter-equations-motion/
    ...
    sh.setODE( 'y'    , x.dy);
    sh.setODE( 'z'    , x.dz);
    sh.setODE( 'dx'   , u.u1*(cos(x.phi)*cos(x.theta)) + u.u2*(sin(x.phi)*sin(x.theta)*cos(x.psi)-cos(x.phi)*sin(x.psi))   + u.u3*(sin(x.phi)*sin(x.psi)+cos(x.phi)*sin(x.theta)*cos(x.psi)));
    sh.setODE( 'dy'   , u.u1*(cos(x.theta)*sin(x.psi)) + u.u2*(cos(x.phi)*cos(x.psi) + sin(x.phi)*sin(x.theta)*sin(x.psi)) + u.u3*(-sin(x.phi)*cos(x.psi)+cos(x.phi)*sin(x.theta)*sin(x.psi)));
    sh.setODE( 'dz'   , u.u1*(-sin(x.theta))           + u.u2*(cos(x.theta)*sin(x.phi))                                    + u.u3*(cos(x.phi)*cos(x.theta)) - c.g);
    ...
    sh.setODE( 'p'    , (1/Ix)*(L + (Iy-Iz)*x.q*x.r));
    sh.setODE( 'q'    , (1/Iy)*(M + (Iz-Ix)*x.r*x.p));
    sh.setODE( 'r'    , (1/Iz)*((Ix-Iy)*x.p*x.q));
    
    sh.setODE( 'm'    , -sqrt((u.u1)^2 + (u.u2)^2 +(u.u3)^2)/(c.Isp*c.g0));
